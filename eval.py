...
import time
import math
from utils.dataset import load_npz, load_single_npz
from utils.traj_speed import cubic_speed, cubic_speed_single
...
def eval_position():
    ...
    pos_pre_data = [0]*3
    t_start = time.time()
    trajs = os.listdir("test_data")
    for traj in trajs:
        for i in range(3): # x,y,z
            pos_raw_data[i] = load_single_npz("test_data",traj, i)
            ...
            speed_data = cubic_speed_single(pos_raw_data[i])
        ...
        if record_err:
            ...
            err = err_cal(np.array(pos_raw_data)[:,1,0,-1],
